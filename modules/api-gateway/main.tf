...
resource "aws_apigatewayv2_integration" "integration" {
  api_id                 = aws_apigatewayv2_api.apigw.id
  integration_uri        = var.sample_invoke_arn
...
resource "aws_apigatewayv2_integration" "bedrock-sample" {
  api_id                 = aws_apigatewayv2_api.apigw.id
  integration_uri        = var.bedrock_sample_invoke_arn
  integration_type       = "AWS_PROXY"
  integration_method     = "POST"
  payload_format_version = "2.0"
}
...
resource "aws_apigatewayv2_integration" "ecs-task-sample" {
  api_id                 = aws_apigatewayv2_api.apigw.id
  integration_uri        = var.ecs_task_sample_invoke_arn
  integration_type       = "AWS_PROXY"
  integration_method     = "POST"
  payload_format_version = "2.0"
}
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = var.sample_function_name
...
resource "aws_lambda_permission" "bedrock-sample" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = var.bedrock_sample_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.apigw.execution_arn}/*/*/*"
}
